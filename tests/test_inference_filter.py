...
# --------------------------------------------------------------------------
# Belief-informing broadcast: choice-independent L/phi observations folded
# into routes the traveller did NOT take (paper Experiment 3: CG/SN).
# --------------------------------------------------------------------------
def _belief_windows(N, W, *, L_route=None, L_val=0.0, phi_route=None, phi_val=0.45):
    """Zero belief windows, optionally activating one route's L or phi obs."""
    yL = jnp.zeros((N, 2, W))
    sL = jnp.ones((N, 2, W))
    mL = jnp.zeros((N, 2, W))
    yphi = jnp.zeros((N, 2, W))
    sphi = jnp.ones((N, 2, W))
    mphi = jnp.zeros((N, 2, W))
    if L_route is not None:
        yL = yL.at[:, L_route, :].set(L_val)
        sL = sL.at[:, L_route, :].set(20.0)
        mL = mL.at[:, L_route, :].set(1.0)
    if phi_route is not None:
        yphi = yphi.at[:, phi_route, :].set(phi_val)
        sphi = sphi.at[:, phi_route, :].set(0.05)
        mphi = mphi.at[:, phi_route, :].set(1.0)
    return dict(
        y_belief_L_window=yL, sigma_belief_L_window=sL, belief_mask_L_window=mL,
        y_belief_phi_window=yphi, sigma_belief_phi_window=sphi,
        belief_mask_phi_window=mphi,
    )


def test_belief_mask_zero_is_noop():
    """All-zero belief masks leave the posterior identical to omitting the
    belief windows entirely."""
    N, W = 3, W_DEFAULT
    state, priors = _init_state(N=N, W=W)
    route, y_tt, sigma_tt, y_L, sigma_L, mask = _empty_window(N, W)
    for s in range(W):
        route, y_tt, sigma_tt, y_L, sigma_L, mask = _fill_window(
            route, y_tt, sigma_tt, y_L, sigma_L, mask, s,
            route_idx=0, tt_val=20.0, L_val=80.0,
        )
    base = window_step(state, priors, route, y_tt, sigma_tt, y_L, sigma_L, mask, W=W)
    with_zero = window_step(
        state, priors, route, y_tt, sigma_tt, y_L, sigma_L, mask, W=W,
        **_belief_windows(N, W),  # all masks zero
    )
    assert jnp.allclose(base.mu, with_zero.mu, atol=1e-6)
    assert jnp.allclose(base.scale_tril, with_zero.scale_tril, atol=1e-6)


def test_belief_L_pulls_unchosen_route():
    """A CG broadcast on the UNCHOSEN route pulls that route's L belief toward
    the broadcast value, while the chosen route's posterior is unchanged."""
    N, W = 3, W_DEFAULT
    state, priors = _init_state(N=N, W=W, L_mu=50.0)
    # Choose route 0 (alpha) every day; route 1 (beta) is never taken.
    route, y_tt, sigma_tt, y_L, sigma_L, mask = _empty_window(N, W)
    for s in range(W):
        route, y_tt, sigma_tt, y_L, sigma_L, mask = _fill_window(
            route, y_tt, sigma_tt, y_L, sigma_L, mask, s,
    ...
    base = window_step(state, priors, route, y_tt, sigma_tt, y_L, sigma_L, mask, W=W)
    # Broadcast a high queue (200) for the unchosen beta route.
    cg = window_step(
        state, priors, route, y_tt, sigma_tt, y_L, sigma_L, mask, W=W,
        **_belief_windows(N, W, L_route=1, L_val=200.0),
    )
    # Beta (unchosen) L belief moves from the 50 prior toward the 200 broadcast.
    assert float(cg.mu[0, 1, L_IDX]) > float(base.mu[0, 1, L_IDX]) + 5.0
    # Beta posterior variance drops (an observation was folded in).
    assert float(_marginal_var(cg, L_IDX)[0, 1]) < float(_marginal_var(base, L_IDX)[0, 1])
    # Alpha (chosen) posterior is untouched by the beta broadcast.
    assert jnp.allclose(cg.mu[:, 0], base.mu[:, 0], atol=1e-5)


def test_belief_phi_gated_to_signalised_route():
    """An SN broadcast on the bypass (non-signalised) is inert; on the
    signalised route it pulls the phi belief."""
    N, W = 3, W_DEFAULT
    state, priors = _init_state(N=N, W=W, phi_mu=0.45)
    route, y_tt, sigma_tt, y_L, sigma_L, mask = _empty_window(N, W)
    # No first-hand observations (all masked off); only the belief broadcast.
    SIG = jnp.asarray([1.0, 0.0])
    # Broadcast phi=0.2 on the signalised route 0 -> belief should move.
    sig0 = window_step(
        state, priors, route, y_tt, sigma_tt, y_L, sigma_L, mask, signalised=SIG, W=W,
        **_belief_windows(N, W, phi_route=0, phi_val=0.2),
    )
    assert float(sig0.mu[0, 0, PHI_IDX]) < 0.40  # moved from 0.45 toward 0.2
    # Broadcast phi on the bypass route 1 -> inert (phi has no role there).
    sig1 = window_step(
        state, priors, route, y_tt, sigma_tt, y_L, sigma_L, mask, signalised=SIG, W=W,
        **_belief_windows(N, W, phi_route=1, phi_val=0.2),
    )
    assert float(sig1.mu[0, 1, PHI_IDX]) == pytest.approx(0.45, abs=1e-3)
